...
void ItemManager::loadDefaultItemMeshes()
{
    ...
    for(unsigned int i=Item::ITEM_FIRST; i<=Item::ITEM_LAST; i++)
    {
        ...
        {
            logfatal("[ItemManager]", "Item model '%s' in items.xml could not be loaded - aborting", name.c_str());
            exit(-1);
        }
        mesh->grab();
        m_item_mesh[i]            = mesh;
...
bool ItemManager::randomItemsForArena(const AlignedArray<btTransform>& pos)
{
    ...
    for (unsigned int i = 0; i < TOTAL_ITEM; i++)
    {
        ...
        while(true)
        {
            ...
            {
                logwarn("[ItemManager]","Can't place more random items! Use default item location.");
                return false;
            }
    ...
    for (unsigned int i = 0; i < TOTAL_ITEM; i++)
    {
        ...
        {
            logwarn("[ItemManager]","Raycast to surface failed from node %d", used_location[i]);
