...
MusicManager::MusicManager()
{
    ...
    if( device == NULL )
    {
        logwarn("MusicManager", "Could not open the default sound device.");
        m_initialized = false;
    }
    else
    {
        ...
        if( context == NULL )
        {
            logwarn("MusicManager", "Could not create a sound context.");
            m_initialized = false;
        }
        else
        {
...
void MusicManager::addMusicToTracks()
{
        ...
        i=m_all_music.begin(); i!=m_all_music.end(); i++)
    {
        if(!i->second)
        {
            logwarn("MusicManager", "Can't find music file '%s' - ignored.",
                      i->first.c_str());
            continue;
        }
